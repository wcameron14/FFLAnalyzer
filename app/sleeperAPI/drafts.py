...
def insert_draft_picks(conn, cur, draft_id):
    ...
    if draft_picks_data is not None:
        for pick in tqdm(draft_picks_data, desc=f"Inserting draft picks data for draft {draft_id}"):
            query = """
            INSERT INTO draft_picks (player_id, picked_by, roster_id, round, draft_slot, pick_no, team, status, sport, position, number, news_updated, last_name, injury_status, first_name, is_keeper, draft_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (player_id, draft_id)
            DO NOTHING;
            """
            pick_data = (pick["player_id"], pick["picked_by"], check_field(pick["roster_id"]), check_field(pick["round"]), check_field(pick["draft_slot"]), check_field(pick["pick_no"]), pick["metadata"]["team"], pick["metadata"]["status"], pick["metadata"]["sport"], pick["metadata"]["position"], check_field(pick["metadata"]["number"]), pick["metadata"]["news_updated"], pick["metadata"]["last_name"], pick["metadata"]["injury_status"], pick["metadata"]["first_name"], pick["is_keeper"], pick["draft_id"])
            try:
